...
(*.[0-9]*)
	...
	if test ! -f /sources/ncurses-6.1/man_alias.sed ; then
cat >>/sources/ncurses-6.1/man_alias.sed <<-CF_EOF2
		s,@DATADIR@,$datadir,g
...
s,@TSET@,tset,g
CF_EOF2
		echo "...made /sources/ncurses-6.1/man_alias.sed"
	fi
	...
	aliases=`sed -f $top_srcdir/man/manlinks.sed $inalias |sed -f /sources/ncurses-6.1/man_alias.sed | sort -u; test $inalias = $nCurses && echo curses`
	...
	sed	-f /sources/ncurses-6.1/man_alias.sed \
		< $i >$TMP
