...
;
#line 83 "GDBM_File.xs"
	GDBM_FILE dbp;
#line 293 "GDBM_File.c"
	GDBM_File	RETVAL;
...
;
	...
	if (SvROK(ST(0)) && sv_derived_from(ST(0), "GDBM_File")) {
	    ...
	    Perl_croak_nocontext("%s: %s is not of type %s",
			...
			"db", "GDBM_File")
;
#line 203 "GDBM_File.xs"
	    DBM_setFilter(db->filter[ix], code);
#line 768 "GDBM_File.c"
    }
    XSRETURN(1);
}
